function varargout = lovison4(varargin)
%LOVISON4  lovison4 (n=2, m=2) test problem (heterogeneous WORK-space wrapper).
%
% INPUT SPACE (SOBOL_DIGIT_OSCILLATORY HETEROGENEITY):
%
%   x1   ∈ [0           , 5608.74     ]   (range: 5608.74     )
%   x2   ∈ [-211007     , 211007      ]   (range: 422014      )
%
% Effective contrast ratio (max range / min range): 225.7267983017113
%
% Pareto information:
...
%
% USAGE:
%   F = lovison4(x)            % Evaluate objectives at point x (nD vector)
%   [lb, ub] = lovison4('bounds')  % Get bounds
%   info = lovison4()          % Get complete problem information
%
% Reference:
%   J. F. A. Madeira,
...
lb_orig = [0;-1];
ub_orig = [6;1];
lb_work = [0;-211007.1342381633];
ub_work = [5608.739480443788;211007.1342381633];
scale_factors = [934.7899134072981;211007.1342381633];
contrast_ratio = 225.7267983017113;

if nargin == 0
    info.name = mfilename;
    info.problem = 'lovison4';
    ...
    info.pareto_note = 'lovison4: No analytical PF documented. Ref: Lovison (2010).';
...
F = lovison4_orig(x_orig);
varargout{1} = F(:);
return
end  % main wrapper function

% -------------------------------------------------------------------------
% Embedded original problem function (verbatim; only renamed to lovison4_orig)
% -------------------------------------------------------------------------
